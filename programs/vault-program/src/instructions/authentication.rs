...
pub fn handler(bunkr: &mut Bunkr, authentication_object: AuthenticationObject) -> Result<()> {
    ...
    validate_checked(bunkr)?;
    
    validate_password(bunkr.current_hash, &password_hash)?;

    let used_time_interval = validate_root(bunkr.init_time, &current_time_interval, root.to_vec(), proof_path, otp_hash)?;

    validate_time(bunkr.last_accessed_interval , &used_time_interval)?;

    bunkr.last_accessed_interval = used_time_interval as u32;
